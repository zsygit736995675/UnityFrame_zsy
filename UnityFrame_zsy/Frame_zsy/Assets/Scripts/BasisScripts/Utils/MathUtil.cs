...
static class MathUtil
{
    ...
    {
        return StrToVec3(str, '\'');
    }
    public static Vector3 StrToVec3(string str, char separator)
    {
    ...
    public static Vector3 GUIPosToMousePos(Vector3 pos)
    {
        Vector3 newPos = pos;
        newPos.y = Screen.height - pos.y;
        return newPos;
    ...
    public static bool IsBoundsInvalide(ref Bounds bd)
    {
		//return FloatEqual(bd.size.x, 0.0f) || FloatEqual(bd.size.y, 0.0f) || FloatEqual(bd.size.z, 0.0f);

		//@zyj 性能优化//
		Vector3 sz = bd.size;
		float f = Mathf.Min( sz.x, sz.y);
		f = Mathf.Min( sz.z, f);
		return FloatEqual(f, 0f);
    }
    ...
    public static string LimitStringByByte(string src, int MaxBytes)
    {
        ...
        for (int i = 0; i < q.Length; ++i)
        {
            ...
            if (t >= MaxBytes)
            {
                //FireEngine.Logger.GetFile( FireEngine.LogFile.Global ).LogWarning("名字太长！\n");
                break;
            }
        }
        return temp;
