...
def delete_file_folder(folder):
	if os.path.isfile(folder):
		...
		for resFile in res_list:
			delete_file_folder(os.path.join(folder, resFile))
		try:
			os.rmdir(folder)
		except:
			print( "clear exp: error"+folder, file=sys.stderr )

def delete_files_from_des_folder(srcfolder,desfolder):
	if not os.path.exists(srcfolder):
		return
	if not os.path.exists(desfolder):
		return
	res_list = os.listdir(srcfolder)
	for file in res_list:
		force_del(os.path.join(desfolder, file))
...
def copy_folder_type(src, dst, type):
	...
	for srcfile in src_list:
		if os.path.splitext(srcfile)[1] == type:
			srcfile = os.path.abspath( os.path.join(src, srcfile) )
